...
import {
    dataTasksFulfilledSelector,
    fetchDataTasks,
    setFinishDay,
    setStartDay,
    setExpandData
} from "./store/dataTasksSlice";
...
function App() {
    ...
    const getStartDateDay = useCallback((arrayStartDates: number[] = []): number => {
        ...
        return DateTime.fromSeconds(Math.min(...arrayStartDates)).startOf("week").toUnixInteger()
    }, [data])
    ...
    useEffect(() => {
        if (data) {
            dispatch(setExpandData(myStartDay))
        }
    }, [data, myStartDay, dispatch])
